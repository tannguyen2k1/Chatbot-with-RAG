...
class IngestionService:
    ...
    def parse_elements(self, elements) -> list[dict[str, Any]]:
        ...
        for el in elements:
            el_type = el.category
            parsed_elements.append({"type": el_type, "text": str(el)})
    ...
    def ingest_file_bytes(self, file_bytes: bytes, filename: str) -> tuple[list[dict[str, Any]], dict[str, Any]]:
        ...
        try:
            ...
            base_metadata = {"filename": filename, "extension": file_ext}

            parsed_elements = self.parse_elements(elements)
            return parsed_elements, base_metadata
        except Exception as e:
            raise RuntimeError(f"Lỗi khi ingest file: {e!s}") from e
        finally:
    ...
    def ingest_db_record(
        self, content: str, source_metadata: dict[str, Any]
        ...
        try:
            ...
            raise RuntimeError(f"Lỗi khi ingest DB record: {e!s}") from e
