...
class UserService:
    ...
    async def update_user(self, user_id: int, update_data: UserUpdate) -> Optional[User]:
        async with TenantSession(bind=engine) as session:
            ...
            if update_data.role is not None:
                ...
                await session.execute(delete(UserRole).filter_by(user_id=user_id))
                # Tìm role id mới theo tenant_id
                result = await session.execute(select(Role).filter_by(name=update_data.role, tenant_id=user.tenant_id))
    ...
    async def update_user_for(self, current_user_id: int, user_id: int, update_data: UserUpdate) -> UserResponse:
        ...
        async with TenantSession(bind=engine) as session:
            ...
            result = await session.execute(select(UserRole).filter_by(user_id=getattr(updated_user, "id", None)))
            user_roles = result.scalars().all()
            role_ids = [ur.role_id for ur in user_roles]
            if role_ids:
                result = await session.execute(select(Role).filter(Role.id.in_(role_ids)))
