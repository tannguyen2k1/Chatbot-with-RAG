...
class RBACService:
    ...
    async def assign_role_to_user_for(self, current_user_id: int, user_id: int, role_id: int):
        ...
        if not is_root_user:
            async with TenantSession(bind=engine) as session:
                result = await session.execute(select(Role).filter_by(id=role_id))
    ...
    async def remove_permission_from_role_for(self, current_user_id: int, role_id: int, module_id: int, permission_id: int):
        ...
        if not is_root_user:
            async with TenantSession(bind=engine) as session:
                result = await session.execute(select(Permission).filter_by(id=permission_id))
    ...
    async def get_role_by_id(self, role_id: int):
        async with TenantSession(bind=engine) as session:
            ...
            result = await session.execute(select(RolePermission).filter_by(role_id=role_id))
    ...
    async def get_module_by_name(self, name: str):
        async with TenantSession(bind=engine) as session:
            result = await session.execute(select(Module).filter_by(name=name))
    ...
    async def delete_role(self, role_id: int):
        async with TenantSession(bind=engine) as session:
            ...
            await session.execute(delete(RolePermission).filter_by(role_id=role_id))
            await session.execute(delete(UserRole).filter_by(role_id=role_id))
    ...
    async def is_root(self, user):
        ...
        async with TenantSession(bind=engine) as session:
            ...
            if not role_ids:
                return False
            result = await session.execute(select(Role).filter(Role.id.in_(role_ids)))
    ...
    async def can_manage_user(self, current_user, target_user):
        ...
        async with TenantSession(bind=engine) as session:
            ...
            result = await session.execute(select(UserRole).filter_by(user_id=current_user.id))
            user_roles = result.scalars().all()
            result = await session.execute(select(UserRole).filter_by(user_id=target_user.id))
            target_roles = result.scalars().all()
            role_ids = [ur.role_id for ur in user_roles]
            target_role_ids = [ur.role_id for ur in target_roles]
            result = await session.execute(select(Role).filter(Role.id.in_(role_ids)))
            roles = result.scalars().all()
            result = await session.execute(select(Role).filter(Role.id.in_(target_role_ids)))
            target_roles = result.scalars().all()
            role_names = [r.name for r in roles]
    ...
    async def get_user_permissions(self, user_id: int) -> dict:
        ...
        async with TenantSession(bind=engine) as session:
            ...
            result = await session.execute(select(UserRole).filter_by(user_id=user_id))
            user_roles = result.scalars().all()
            role_ids = [ur.role_id for ur in user_roles]
            if not role_ids:
                return permissions_dict
            # Lấy tất cả RolePermission của các role này
            result = await session.execute(select(RolePermission).filter(RolePermission.role_id.in_(role_ids)))
            role_perms = result.scalars().all()
    ...
    async def remove_permission_from_role(self, role_id: int, module_id: int, permission_id: int):
        async with TenantSession(bind=engine) as session:
            result = await session.execute(select(RolePermission).filter_by(role_id=role_id, module_id=module_id, permission_id=permission_id))
    ...
    async def get_all_permissions(self):
        ...
        async with TenantSession(bind=engine) as session:
            ...
            result = []
            result_modules = await session.execute(select(Module))
    ...
    async def assign_permission_to_role(self, role_id: int, module_id: int, permission_id: int):
        async with TenantSession(bind=engine) as session:
            ...
            result = await session.execute(select(RolePermission).filter_by(
                role_id=role_id, 
                module_id=module_id, 
            ...
            # Create new permission assignment with tenant_id
            rp = RolePermission(role_id=role_id, module_id=module_id, permission_id=permission_id, tenant_id=role.tenant_id)
            session.add(rp)
            await session.commit()
    ...
    async def check_user_permission(self, user_id: int, module_name: str, permission_name: str) -> bool:
        async with TenantSession(bind=engine) as session:
            ...
            result = await session.execute(select(Module).filter_by(name=module_name))
            module = result.scalar_one_or_none()
            result = await session.execute(select(Permission).filter_by(name=permission_name))
            permission = result.scalar_one_or_none()
            if not module or not permission:
                return False
            result = await session.execute(select(RolePermission).filter(
