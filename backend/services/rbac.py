...
from database.models import Module, Permission, Role, RolePermission, UserRole
from schemas import PermissionError, RoleUpdate
...
class RBACService:
    ...
    async def remove_permission_from_role_for(
        self, current_user_id: int, role_id: int, module_id: int, permission_id: int
    ):
    ...
    async def assign_permission_to_role_for(
        self, current_user_id: int, role_id: int, module_id: int, permission_id: int
    ):
    ...
    async def check_user_permission_for(
        self, current_user_id: int, user_id: int, module_name: str, permission_name: str
    ) -> bool:
    ...
    async def get_role_by_id(self, role_id: int):
        ...
        perms = [{"module_id": rp.module_id, "permission_id": rp.permission_id} for rp in role_perms]
        return {
            "id": role.id,
            "name": role.name,
    ...
    async def can_manage_user(self, current_user, target_user):
        ...
        if "admin" in role_names:
            return not any(r in ["admin", "root"] for r in target_role_names)
    ...
    async def remove_permission_from_role(self, role_id: int, module_id: int, permission_id: int):
        result = await self.db.execute(
            select(RolePermission).filter_by(role_id=role_id, module_id=module_id, permission_id=permission_id)
        )
    ...
    async def get_all_roles(self):
        ...
        for rp in role_permissions:
            role_perm_map[rp.role_id].append({"module_id": rp.module_id, "permission_id": rp.permission_id})
