...
from schemas.audit_log import AuditLogOut, PaginatedAuditLogResponse
...
class AuditLogService:
    ...
    async def get_all_audit_logs(self, page: int = 1, page_size: int = 10, search: str | None = None):
        """Lấy tất cả audit_logs với phân trang và tìm kiếm"""
        query = select(AuditLog)
        if search:
            like = f"%{search}%"
            query = query.filter(
                (AuditLog.table_name.ilike(like)) | (AuditLog.action.ilike(like)) | (AuditLog.description.ilike(like))
            )
        # Get total count
    ...
    async def get_all_audit_logs_for(
